...
def make_input():
    rows = pd.DataFrame({"a": [1, 2, 3, 4], "b": [5, 6, 7, 8], "target": [0, 1, 0, 1]})
    train = rows.iloc[[0, 1, 2]].reset_index(drop=True)
    test = rows.iloc[[2, 3]].reset_index(drop=True)  # row 2 overlaps
...
def make_input():
    train = pd.DataFrame({"a": [1, 2, 3], "target": [0, 1, 0]})
    test = pd.DataFrame({"a": [4, 5, 6, 7], "target": [1, 0, 1, 0]})
    return DataAuditInput(train=train, test=test, target="target")
""",
)
...
def make_input():
    train = pd.DataFrame({"x": [1, 2, 3], "ts": pd.to_datetime(["2020-01-01", "2020-06-01", "2021-01-01"]), "target": [0, 1, 0]})
    test = pd.DataFrame({"x": [4, 5], "ts": pd.to_datetime(["2020-03-01", "2021-06-01"]), "target": [1, 0]})
...
def run():
    ...
    # mirroring how `leakproof run script.py` executes user code under the hooks.
    from sklearn.linear_model import LogisticRegression
    from sklearn.model_selection import train_test_split
